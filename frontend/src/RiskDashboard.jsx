...
import {
  ...
  Crosshair, Scale, LogOut,
} from "lucide-react";

/* ═══════════════════════════════════════════════════════════════
   BUNDLED SNAPSHOT — real output of run_dashboard.py --json
   (regenerate with:
    python run_dashboard.py --capital 10 --loops 4 --simulations 2000 \
      --horizon 7 --seed 42 --market-regime-forecast --json \
      > frontend/src/demo_payload.json)
   ═══════════════════════════════════════════════════════════════ */
import SNAPSHOT_DATA from "./demo_payload.json";
...
async function fetchDashboardData() {
  ...
  try {
    ...
    if (resp.status === 202 || resp.status === 502 || resp.status === 504) {
      /* 202: simulation still running (API dedup). 502/504: the proxy gave
         up waiting but the backend keeps computing and caches the result —
         either way the caller should keep polling. */
    ...
    if (err?.name === "AbortError") {
      throw new Error(
        "Simulation in progress — the backend is still computing. Retrying..."
      );
    }
    throw err;
  } finally {
  ...
  return (
    <header className="w-full bg-[#111318] border-b border-[rgba(255,255,255,0.06)]">
      ...
      <div className="px-6 py-4 flex items-center gap-8 overflow-x-auto">
        ...
        <VitalPill label={`VaR₉₅ (${rm.horizon_days}d)`} value={fmtEth(rm.var_95_eth)} color="#f0b429" />
...
function BorrowRateFanChart() {
  ...
  const rm = DATA.risk_metrics;
  /* Steps can be sub-daily (operational profile is 10-minute); convert
     step index to days so the x-axis is honest at any resolution. */
  const horizonDays = Number(rm.horizon_days) || fan["50"].length - 1;
  const daysPerStep = horizonDays / Math.max(fan["50"].length - 1, 1);
  const chartData = fan["50"].map((_, i) => ({
    day: Number((i * daysPerStep).toFixed(2)),
    p5: fan["5"][i],
    p25: fan["25"][i],
    p50: fan["50"][i],
    p75: fan["75"][i],
  ...
  return (
    <Panel title={`Borrow Rate Fan Chart — ${fmtNum(horizonDays, 0)} Day Forecast`} icon={Activity} accentColor="#f0b429"
      rightSlot={
        <span className="font-mono text-[10px] text-txt-muted">
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 20, bottom: 10, left: 10 }}>
            ...
            <XAxis
              dataKey="day" type="number" domain={[0, horizonDays]}
              tickFormatter={(v) => Number(v).toFixed(horizonDays <= 3 ? 1 : 0)}
              tick={{ fill: "#6b7280", fontFamily: "'JetBrains Mono', monospace", fontSize: 10 }}
  ...
  return (
    <div className="flex items-center gap-2">
      {isLive ? (
        ...
        <span
          className="flex items-center gap-1 font-mono text-[10px] px-2 py-0.5 bg-[rgba(240,180,41,0.08)] text-[#f0b429] border border-[rgba(240,180,41,0.15)]"
          title="Bundled output of a real simulation run; refreshes to live API results automatically"
        >
          <Database size={10} /> SNAPSHOT
        </span>
      )}
      {fetchedAt && (
        <span className="font-mono text-[10px] text-txt-muted">
...
/* ═══════════════════════════════════════════════════════════════
   SECTION: TRADING MODEL — touch forecast, sizing, exit policy
   ═══════════════════════════════════════════════════════════════ */
function PanelUnavailable({ reason }) {
  return (
    <div className="font-mono text-[11px] text-txt-muted py-6 text-center">
      unavailable{reason ? ` — ${reason}` : ""}
    </div>
  );
}

function TouchForecastPanel() {
  const DATA = useData();
  const touch = DATA.professional_modeling?.touch_model_forecast;

  const horizons = touch?.status === "available" ? touch.horizons : [];
  const history = touch?.history;

  return (
    <Panel
      title="Touch Probability Model"
      icon={Crosshair}
      accentColor="#2dd4bf"
      rightSlot={
        touch?.status === "available" ? (
          <span className="font-mono text-[10px] px-2 py-0.5 bg-[rgba(45,212,191,0.08)] text-[#2dd4bf] border border-[rgba(45,212,191,0.15)]">
            GATED · OOS EDGE CONFIRMED
          </span>
        ) : null
      }
    >
      {touch?.status !== "available" ? (
        <PanelUnavailable reason={touch?.reason} />
      ) : (
        <div className="space-y-4">
          <div className="font-sans text-[11px] text-txt-muted leading-relaxed">
            Walk-forward logistic model of first-touch probabilities on real
            Deribit ETH-PERPETUAL history. Persisted only when the
            out-of-sample Brier gate beats climatology by ≥5%.
          </div>
          {horizons.map((h) => {
            const rows = h.targets.map((t) => ({
              label: `${t.direction === "down" ? "−" : "+"}${Math.abs(
                (t.target_multiplier - 1) * 100
              ).toFixed(0)}% ($${Math.round(t.target_eth_usd).toLocaleString()})`,
              pct: t.first_touch_probability_pct,
              direction: t.direction,
            }));
            const isPrimary = h.horizon_hours === touch.primary_horizon;
            return (
              <div key={h.horizon_hours}>
                <div className="flex items-baseline justify-between mb-1">
                  <span className="font-mono text-[11px] text-txt-secondary uppercase tracking-[0.08em]">
                    {h.horizon_hours / 24}d horizon{isPrimary ? " · primary" : ""}
                  </span>
                  <span className="font-mono text-[10px] text-txt-muted">
                    gate {h.walk_forward_gate.brier_improvement_pct >= 0 ? "+" : ""}
                    {fmtNum(h.walk_forward_gate.brier_improvement_pct, 2)}% Brier vs climatology
                  </span>
                </div>
                <ResponsiveContainer width="100%" height={rows.length * 30 + 10}>
                  <BarChart data={rows} layout="vertical" margin={{ top: 0, right: 40, bottom: 0, left: 8 }}>
                    ...
                    >
                      {rows.map((r, i) => (
                        <Cell key={i} fill={r.direction === "down" ? "#f97316" : "#2dd4bf"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <div className="font-mono text-[10px] text-txt-muted border-t border-[rgba(255,255,255,0.06)] pt-2 space-y-0.5">
            <div>
              mark ${fmtNum(history.mark_price_usd, 0)} · as of {fmtTimestamp(history.asof_utc)}
              {history.stale && <span className="text-[#ef4444]"> · STALE HISTORY</span>}
            </div>
            <div>
              strict per-target skill is small — scenario weights, not directional alpha
            </div>
          </div>
        </div>
      )}
...
function PositionSizingPanel() {
  const DATA = useData();
  const sizing = DATA.professional_modeling?.position_sizing;

  const bindingLabel = {
    fractional_kelly: "fractional Kelly binds",
    cvar_budget: "CVaR budget binds",
    both_equal: "Kelly and CVaR agree",
    non_positive_expected_log_growth: "no positive expected log growth",
  };

  return (
    <Panel title="Position Sizing" icon={Scale} accentColor="#f0b429">
      {sizing?.status !== "available" ? (
        <PanelUnavailable reason={sizing?.reason} />
      ) : (
        <div className="space-y-4">
          <div className="flex items-end justify-between">
            <MetricCard
              label="Recommended loops"
              value={
                sizing.recommended_loops === 0
                  ? "0 — stay out"
                  : sizing.recommended_loops
              }
              color={sizing.recommended_loops === 0 ? "#f97316" : "#2dd4bf"}
              sublabel={bindingLabel[sizing.binding_constraint] || sizing.binding_constraint}
            />
            <div className="text-right space-y-1">
              <div className="font-mono text-[10px] text-txt-muted">
                growth-optimal: {sizing.growth_optimal.loops} loops
                ({sizing.growth_optimal.expected_log_growth >= 0 ? "+" : ""}
                {fmtNum(sizing.growth_optimal.expected_log_growth, 5)} E[log g])
              </div>
              <div className="font-mono text-[10px] text-txt-muted">
                ½-Kelly target ≤ {fmtNum(sizing.fractional_kelly.target_leverage, 3)}× lev
                · CVaR₉₅ budget {fmtNum(sizing.cvar_budget.budget_eth, 2)} ETH
              </div>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={190}>
            <ComposedChart
              data={sizing.candidates.map((c) => ({
                loops: c.loops,
                growth: c.log_growth_annualized * 100,
                cvar: c.cvar95_pct_of_capital,
                passes: c.passes_constraints,
              }))}
            ...
            >
              ...
              <XAxis
                dataKey="loops"
                tick={{ fill: "#9ca3af", fontSize: 10, fontFamily: "JetBrains Mono" }}
                label={{ value: "loops", position: "insideBottomRight", offset: -2, fill: "#6b7280", fontSize: 10 }}
              />
              <YAxis
                yAxisId="growth"
                tick={{ fill: "#9ca3af", fontSize: 10, fontFamily: "JetBrains Mono" }}
              ...
              />
              <YAxis yAxisId="cvar" orientation="right" hide />
              <Tooltip
                content={({ active, payload, label }) =>
                  active && payload?.length ? (
                    <div className="bg-[#1a1d24] border border-[rgba(255,255,255,0.12)] px-3 py-2 shadow-xl font-mono text-xs space-y-0.5">
                      <div className="text-txt-secondary text-[10px]">{label} loops</div>
                      <div className="text-txt-primary">E[log g] ann.: {fmtNum(payload[0]?.payload.growth, 2)}%</div>
                      <div className="text-txt-primary">CVaR₉₅: {fmtNum(payload[0]?.payload.cvar, 1)}% of capital</div>
                      <div className="text-txt-muted text-[10px]">
                        {payload[0]?.payload.passes ? "passes constraints" : "fails constraints"}
                      </div>
                    </div>
                  ) : null
                }
              />
              <ReferenceLine yAxisId="growth" y={0} stroke="rgba(255,255,255,0.15)" />
              <Bar yAxisId="growth" dataKey="growth" name="E[log growth] (ann.)" barSize={18}>
                {sizing.candidates.map((c, i) => (
                  <Cell
                    key={i}
                    fill={
                      c.loops === sizing.growth_optimal.loops
                        ? "#f0b429"
                        : c.passes_constraints
                        ? "#2dd4bf"
                        : "#4b5563"
                    }
                  />
          ...
          <div className="font-mono text-[10px] text-txt-muted border-t border-[rgba(255,255,255,0.06)] pt-2">
            empirical Kelly on simulated terminal P&L · gold bar = growth-optimal ·
            orange line = CVaR₉₅ (% capital)
            {sizing.fractional_kelly.forecast_tilt?.source === "supervised_touch_model" && (
              <> · touch-model tilt ×{fmtNum(sizing.fractional_kelly.forecast_tilt.multiplier, 2)}</>
            )}
          </div>
        </div>
      )}
...
function ExitPolicyPanel() {
  const DATA = useData();
  const exitPolicy = DATA.professional_modeling?.exit_policy;

  return (
    <Panel title="Exit / Deleveraging Policy" icon={LogOut} accentColor="#f97316">
      {exitPolicy?.status !== "available" ? (
        <PanelUnavailable reason={exitPolicy?.reason} />
      ) : (
        <div className="space-y-4">
          <div>
            <div className="font-mono text-[10px] tracking-[0.08em] uppercase text-txt-secondary mb-2">
              HF-triggered partial deleverage ladder
            </div>
            <div className="space-y-1.5">
              {exitPolicy.rungs.map((rung, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between font-mono text-xs bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] px-3 py-1.5"
                >
                  <span className="text-txt-primary">
                    HF &lt; {fmtNum(rung.hf_trigger, 3)} → repay {fmtNum(rung.deleverage_fraction * 100, 0)}% of debt
                  </span>
                  <span className="text-txt-muted text-[10px]">
                    fired on {fmtNum(rung.fired_path_pct, 1)}% of paths
                    {rung.median_fire_day != null && ` · median day ${fmtNum(rung.median_fire_day, 1)}`}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="font-mono text-[10px] tracking-[0.08em] uppercase text-txt-secondary mb-2">
              Policy vs. do-nothing (same simulated paths)
            </div>
            <table className="w-full font-mono text-xs">
              <thead>
                <tr className="text-[10px] text-txt-muted uppercase">
                  <th className="text-left font-normal pb-1"></th>
                  <th className="text-right font-normal pb-1">no policy</th>
                  <th className="text-right font-normal pb-1">with ladder</th>
                </tr>
              </thead>
              <tbody className="text-txt-primary">
                <tr>
                  <td className="py-0.5 text-txt-secondary">P(HF &lt; 1)</td>
                  <td className="text-right">{fmtPct(exitPolicy.without_policy.prob_hf_lt_1_pct)}</td>
                  <td className="text-right">{fmtPct(exitPolicy.with_policy.prob_hf_lt_1_pct)}</td>
                </tr>
                <tr>
                  <td className="py-0.5 text-txt-secondary">min HF (p5)</td>
                  <td className="text-right">{fmtHf(exitPolicy.without_policy.min_hf_p5)}</td>
                  <td className="text-right">{fmtHf(exitPolicy.with_policy.min_hf_p5)}</td>
                </tr>
                <tr>
                  <td className="py-0.5 text-txt-secondary">terminal P&amp;L mean</td>
                  <td className="text-right">{fmtEth(exitPolicy.without_policy.terminal_pnl.mean_eth, 3)}</td>
                  <td className="text-right">{fmtEth(exitPolicy.with_policy.terminal_pnl.mean_eth, 3)}</td>
                </tr>
                <tr>
                  <td className="py-0.5 text-txt-secondary">CVaR₉₅</td>
                  <td className="text-right">{fmtEth(exitPolicy.without_policy.terminal_pnl.cvar95_eth, 3)}</td>
                  <td className="text-right">{fmtEth(exitPolicy.with_policy.terminal_pnl.cvar95_eth, 3)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="font-mono text-[10px] text-txt-muted border-t border-[rgba(255,255,255,0.06)] pt-2 space-y-0.5">
            <div>
              start HF {fmtHf(exitPolicy.start_health_factor)} ·{" "}
              {exitPolicy.ladder_source === "default_scaled_to_entry_hf_buffer"
                ? "rungs scaled to entry HF buffer"
                : "custom ladder from request"}
            </div>
            <div>
              triggered on {fmtNum(exitPolicy.trigger_summary.any_rung_fired_path_pct, 1)}% of paths
              {exitPolicy.trigger_summary.any_rung_fired_path_pct > 0 &&
                ` · mean cost when triggered ${fmtNum(
                  exitPolicy.trigger_summary.mean_deleverage_cost_when_triggered_eth,
                  4
                )} ETH`}
              {" · sells at market stETH/ETH with Curve slippage + gas"}
            </div>
          </div>
        </div>
      )}
...
export default function RiskDashboard() {
  ...
  const loadFromApi = useCallback(async (isRefresh = false) => {
    ...
    /* A cold backend run (live fetches + Monte Carlo) can take minutes;
       the API answers 202 while a duplicate request is computing. Poll
       patiently — the snapshot keeps the page usable meanwhile. */
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < MAX_RETRIES && err.message.includes("in progress")) {
          setRetryMsg(`Live simulation running on backend... (${attempt + 1})`);
  ...
  /* On mount: render the bundled snapshot instantly, then swap in live
     API results in the background once the backend finishes. */
  useEffect(() => {
    setData(SNAPSHOT_DATA);
    setDataSource("snapshot");
    setLoading(false);
    ...
    <DataContext.Provider value={data}>
      <div className="min-h-screen" style={{ background: "#0a0b0d" }}>
        {isRefreshing && data && (
          <div className="px-6 py-2 bg-[rgba(240,180,41,0.08)] border-b border-[rgba(240,180,41,0.2)]">
            <div className="max-w-[1600px] mx-auto font-mono text-[11px] text-[#f0b429] flex items-center gap-2">
              ...
              <span>{retryMsg || "Running live simulation in background — showing bundled snapshot meanwhile..."}</span>
            </div>
          </div>
        )}
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-6">
          ...
          {/* Section H: Trading model — forecast, sizing, exit policy */}
          {data?.professional_modeling && (
            <>
              <div className="mt-6">
                <GoldRule />
              </div>
              <div className="mt-6">
                <div className="flex items-center gap-2 mb-4">
                  <Activity size={14} className="text-txt-secondary" />
                  <span className="font-mono text-xs font-medium tracking-[0.1em] uppercase text-txt-secondary">
                    Trading Model — Forecast, Sizing &amp; Exit Policy
                  </span>
                </div>
          ...
          {/* Section I: Methodology (full width) */}
          <div className="mt-6">
            <MethodologySection />
          </div>
        </div>
